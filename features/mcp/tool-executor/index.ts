...
export class ToolExecutor implements ToolExecutorContract {
  ...
  constructor(private deps: ToolExecutorDependencies) {}
  ...
  private async discoverToolsFromServer(serverName: string): Promise<ToolDefinition[]> {
  ...
  private async executeToolInternal(params: any, executionId: string): Promise<ExecutionResult> {
  ...
  private validateToolArguments(tool: ToolDefinition, args: Record<string, unknown>): void {
  ...
  async getExecutionHistory(limit?: number): Promise<ExecutionResult[]> {
  ...
  async getToolDefinition(serverName: string, toolName: string): Promise<ToolDefinition | null> {
