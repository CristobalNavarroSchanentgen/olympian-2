 ...
 */
export class ReasoningDataAdapter {
  
  /**
   * Transform raw text reasoning into structured blocks
   */
  transformTextToBlocks(text: string, type: ReasoningBlock['type'] = 'step'): Omit<ReasoningBlock, 'id' | 'timestamp' | 'stepNumber'>[] {
  ...
  transformAIReasoning(reasoning: any): Omit<ReasoningBlock, 'id' | 'timestamp' | 'stepNumber'>[] {
    ...
    const blocks: Omit<ReasoningBlock, 'id' | 'timestamp' | 'stepNumber'>[] = [];
