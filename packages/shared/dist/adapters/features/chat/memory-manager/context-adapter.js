...
function createContextAdapter() {
    return {
        buildConversationContext(messages, maxTokens) {
            const context = (0, context_manager_1.buildContext)(messages, {
                maxTokens,
        ...
        optimizeContext(context, targetTokens) {
            ...
            const optimized = (0, context_manager_1.optimizeContext)(context.messages, {
                targetTokens,
                preserveRecent: true,
        ...
        analyzeContextQuality(context) {
            ...
            const completeness = ((hasSystemMessage ? 0.3 : 0) +
            ...
            const suggestions = [];
            if (completeness < 0.8)
                suggestions.push('Consider including more diverse message types');
            if (relevance < 0.7)
                suggestions.push('Focus on more recent conversation context');
            if (coherence < 0.8)
                suggestions.push('Improve conversation flow preservation');
