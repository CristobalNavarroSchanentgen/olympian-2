...
const capability_detector_1 = require("../../../../utils/capability-detector");
function createCapabilityScannerAdapter() {
    return {
        async scanModelCapabilities(modelName, endpoint) {
            ...
            try {
                const capabilities = await (0, capability_detector_1.detectCapabilities)(modelName, {
                    endpoint,
                    timeout: 30000,
                    methods: [
                        'model-info',
        ...
        async batchScanCapabilities(models, endpoint) {
            ...
            for (let i = 0; i < models.length; i += concurrency) {
                ...
                const batchResults = await Promise.all(batch.map(model => this.scanModelCapabilities(model, endpoint)));
        ...
        async testVisionCapability(modelName, endpoint) {
            try {
                const visionTest = await (0, capability_detector_1.detectCapabilities)(modelName, {
                    endpoint,
                    timeout: 15000,
                    methods: ['vision-test'],
                    testImage: true
        ...
        detectOptimalSettings(capability) {
            ...
            // Safe access to metadata properties with type guards
            ...
            // Safe access to size property with type guard
