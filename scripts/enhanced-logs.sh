...
stream_all_logs() {
    ...
    # Create named pipes for different log sources
    mkfifo /tmp/docker_logs /tmp/browser_logs 2>/dev/null || true
    
    # Start Docker logs in background
    docker-compose logs --no-color -f > /tmp/docker_logs &
    DOCKER_PID=$!
    
    # Start browser logs polling in background
    (
        while true; do
    ...
    (
        tail -f /tmp/docker_logs | while read line; do
            echo -e "${BLUE}[DOCKER]${NC} $line"
        done
    ) &
    
    (
        tail -f /tmp/browser_logs | while read line; do
            if [[ $line == *"[error]"* ]]; then
                echo -e "${RED}[BROWSER]${NC} $line"
            elif [[ $line == *"[warn]"* ]]; then
                echo -e "${YELLOW}[BROWSER]${NC} $line"
            else
                echo -e "${CYAN}[BROWSER]${NC} $line"
            fi
        done
    ) &
    
    # Wait for interrupt
    trap 'kill $DOCKER_PID $BROWSER_PID 2>/dev/null; rm -f /tmp/docker_logs /tmp/browser_logs; exit 0' INT
    wait
}
